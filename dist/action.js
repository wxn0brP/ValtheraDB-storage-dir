...
export class FileActions extends ActionsBase {
     ...
     */
    _validateTransaction(query) {
        if (this.journal.isPoisoned()) {
            throw new Error("journal is poisoned from previous failed commit, cannot perform operations");
        }
        const txHandle = query.transaction;
        if (this.activeTx) {
            if (!txHandle || txHandle.id !== this.activeTx.id) {
                throw new Error("transaction is active but operation has no or mismatched handle");
            }
        }
    }
    _getOpts() {
    ...
    async add(query) {
        ...
        const c_path = this._getCollectionPath(collection);
        const file = c_path +
            (await this.utils.getLastFile(c_path, this.options.maxFileSize, query, this._getOpts()));
    ...
    async findOne(query) {
        ...
        await this.ensureCollection(collection);
        const c_path = this._getCollectionPath(collection);
        const files = await this.utils.getSortedFiles(c_path, query, this._getOpts());
        for (const f of files) {
            const data = (await this.fileCpu.findOne(c_path + f, query, this._getOpts()));
            if (data)
                return data;
        }
        return null;
    ...
    async update(query) {
        ...
        this._ensureQueryFormat(query);
        await this.ensureCollection(collection);
        return await this.utils.operationUpdater(this._getCollectionPath(collection), this.fileCpu.update.bind(this.fileCpu), false, query, this._getOpts());
    }
    /**
    ...
    async updateOne(query) {
        ...
        this._ensureQueryFormat(query);
        await this.ensureCollection(collection);
        const res = await this.utils.operationUpdater(this._getCollectionPath(collection), this.fileCpu.update.bind(this.fileCpu), true, query, this._getOpts());
        return res[0] ?? null;
    }
    /**
    ...
    async remove(query) {
        ...
        this._ensureQueryFormat(query);
        await this.ensureCollection(query.collection);
        return await this.utils.operationUpdater(this._getCollectionPath(collection), this.fileCpu.remove.bind(this.fileCpu), false, query, this._getOpts());
    }
    /**
    ...
    async removeOne(query) {
        ...
        this._ensureQueryFormat(query);
        await this.ensureCollection(query.collection);
        const res = await this.utils.operationUpdater(this._getCollectionPath(collection), this.fileCpu.remove.bind(this.fileCpu), true, query, this._getOpts());
        return res[0] ?? null;
    }
    /**
    ...
    async beginTransaction(id) {
        ...
        if (this.journal.isPoisoned()) {
            throw new Error("journal is poisoned from previous failed commit, cannot begin new transaction");
        }
        const handle = {
            id,
        };
