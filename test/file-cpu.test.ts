...
import { FileCpuOpts } from "../src/types";
...
describe("file CPU operations", () => {
	...
	describe("find", () => {
		...
		it("1. should find all entries with empty search", async () => {
			const results = await vFileCpu.find(
				TEST_FILE,
				{
					collection: "test",
					search: {},
	...
	describe("findOne", () => {
		...
		it("1. should return first matching entry", async () => {
			const result = await vFileCpu.findOne(
				TEST_FILE,
				{
					collection: "test",
					search: {
	...
	describe("update", () => {
		...
		it("1. should update all entries with empty search", async () => {
			const updated = await vFileCpu.update(
				TEST_FILE,
				{
					collection: "test",
					search: {},
		...
		it("4. should update only first entry when one=true", async () => {
			...
			expect(results.filter((item: any) => item.updated === true).length).toBe(
				1,
			);
		});
	...
	describe("remove", () => {
		...
		it("1. should remove all entries with empty search", async () => {
			const removed = await vFileCpu.remove(
				TEST_FILE,
				{
					collection: "test",
					search: {},
			...
			const remaining = await vFileCpu.find(
				TEST_FILE,
				{
					collection: "test",
					search: {},
